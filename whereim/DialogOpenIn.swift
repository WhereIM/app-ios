...
class DialogOpenIn {
    init(_ viewController: UIViewController, _ location: CLLocationCoordinate2D, _ title: String?, _ view: UIView, _ touchPosition: CGPoint?) {
        ...
        let apple_maps = UIAlertAction(title: "apple_maps".localized, style: .default) { (alert: UIAlertAction!) -> Void in
        ...
        if (UIApplication.shared.canOpenURL(google_maps_url)) {
            let google_maps = UIAlertAction(title: "google_maps".localized, style: .default) { (alert: UIAlertAction!) -> Void in
        ...
        if (UIApplication.shared.canOpenURL(google_navigation_url)) {
            let google_navigation = UIAlertAction(title: "google_maps_navigation".localized, style: .default) { (alert: UIAlertAction!) -> Void in
